    ...
    public RedBottomHighPreset(Elevator elevator, Manipulator manipulator) {
            ...
            addCommands(
                    new ParallelCommandGroup(
                            new InstantCommand(() -> manipulator.setArm(0.47)),
                            new TurretToPosition(manipulator, Constants.Manipulator.Turret.LEFT_MAXIMUM_POSITION + 350, 1),
                            new InstantCommand(() -> manipulator.setPusher(Constants.Manipulator.Pusher.PUSHER_PREP_POS))
                    ),
                    new ElevatorToPosition(elevator, 725, 1)
            );
